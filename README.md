...
Firstly, use NecessaryLibraries.R to ensure all libraries are installed or loaded, and set the working directory to the one in use. Secondly, using BaseFunctions.R, store each function in the environment. Navigating back to NecessaryLibraries.R, run Import_FIRMS (this usually takes a minute or two). Finally, set the TFIM parameters to the desired latitude, longitude, date range, etc.

Running the code in DataCleaning.R will import and organize the Oil Sands data into a cleaner, optimal form for plotting.

### Plotting 
All scripts used to create the plots I have made are contained in PlottingCode.Rmd, each plot receives its own chunk with a title explaining said plot.
